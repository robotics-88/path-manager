...
template <typename T> double squared(T x) {
    return x * x;
}

// Returns a weighted average of start and end, where ratio is the weight of
// start
inline double interpolate(double start, double end, double ratio) {
...
template <typename P> void setPointCoordinates(P &point, double x, double y, double z) {
    point.x = x;
    point.y = y;
    point.z = z;
}
...
template <typename P> double norm(const P &p) {
    return norm(p.x, p.y, p.z);
}

// Returns a point between p1 and p2, ratio should be between 0 and 1
// ratio=0 -> p1, ratio=1 -> p2
template <typename P> P interpolate(const P &p1, const P &p2, double ratio) {
    P new_point;
...
template <typename P> P middlePoint(const P &p1, const P &p2) {
    return interpolate(p1, p2, 0.5);
}

template <typename P1, typename P2> P1 addPoints(const P1 &p1, const P2 &p2) {
    P1 new_p;
    new_p.x = p1.x + p2.x;
    new_p.y = p1.y + p2.y;
    new_p.z = p1.z + p2.z;
...
template <typename P, typename Float> P scalePoint(const P &point, Float scalar) {
    P new_p;
